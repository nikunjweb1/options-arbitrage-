...
class ScreenResult:
    ...
    shark_error: str | None = None  # e.g. "expired" -- surfaced, not hidden
...
def run_screen(
    ...
    # Fetch both sides CONCURRENTLY -- see module docstring's SPEED note.
    ...
    if expired_count == len(delta_contracts) and delta_contracts:
        logger.warning(
            "ALL %d strikes came back 'Symbol expired' on Shark for expiry %s -- that whole "
            "expiry has settled on Shark's side. Try the next expiry date.",
            expired_count, shark_expiry_ddmmmyy,
        )
    elif expired_count > 0:
        logger.info("%d/%d strikes expired on Shark (rest are live).", expired_count, len(delta_contracts))
...
def run_watch(underlying: str, shark_expiry: str, delta_expiry: datetime, interval_sec: int, ws_listen_seconds: int) -> None:
    ...
    while True:
        ...
        try:
            results = run_screen(underlying, shark_expiry, delta_expiry, ws_listen_seconds)
        except Exception as exc:  # noqa: BLE001
...
if __name__ == "__main__":
    ...
    if args.watch:
        run_watch(args.underlying, args.shark_expiry, delta_expiry, args.interval_sec, args.ws_listen_seconds)
    else:
        results = run_screen(args.underlying, args.shark_expiry, delta_expiry, args.ws_listen_seconds)
